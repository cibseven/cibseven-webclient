<template>
	<div class="container-fluid pt-4 d-flex flex-column h-100">
		...
		<div class="mb-3">
			<label class="form-label fw-bold">{{ $t('admin.system.execution-metrics.startDateContract') + ':' }}</label>
			<b-form-datepicker class="col-4" v-model="subsDate" :date-disabled-fn="shouldDisableDate" today-button>
		...
		<div v-if="!loading">
			...
			<div class="pb-3">
				<FlowTable striped resizable thead-class="sticky-header" :items="monthlyItems" primary-key="index" prefix="admin.system.execution-metrics." 
				:fields="[
					{ label: '', key: 'month', class: 'col-6', sortable: false, thClass: 'border-end', tdClass: 'border-end py-1 border-top-0' },
					{ label: 'process-instances', key: 'process-instances', sortable: false, class: 'col-2', thClass: 'border-end', tdClass: 'border-end py-1 border-top-0' },
					{ label: 'decision-instances', key: 'decision-instances', sortable: false, class: 'col-2', thClass: 'border-end', tdClass: 'border-end py-1 border-top-0' },
					{ label: 'task-users', key: 'task-users', class: 'col-2', sortable: false, tdClass: 'py-1 border-top-0' }]">
				</FlowTable>
			</div>
			<div class="pb-3">
				<h5>{{ $t('admin.system.execution-metrics.annualUsageBySubscriptionTerm') }}</h5>
				<FlowTable striped resizable thead-class="sticky-header" :items="yearlyItems" primary-key="index" prefix="admin.system.execution-metrics." 
				:fields="[
					{ label: '', key: 'year', class: 'col-6', sortable: false, thClass: 'border-end', tdClass: 'border-end py-1 border-top-0' },
					{ label: 'process-instances', key: 'process-instances', sortable: false, class: 'col-2', thClass: 'border-end', tdClass: 'border-end py-1 border-top-0' },
					{ label: 'decision-instances', key: 'decision-instances', sortable: false, class: 'col-2', thClass: 'border-end', tdClass: 'border-end py-1 border-top-0' },
					{ label: 'task-users', key: 'task-users', class: 'col-2', sortable: false, tdClass: 'py-1 border-top-0' }]">
				</FlowTable>
		...
		<div v-else class="py-3 text-center w-100">
			<BWaitingBox class="d-inline me-2" styling="width: 50px"></BWaitingBox> {{ $t('admin.loading') }}
		</div>
	</div>
</template>
...
export default {
	...
	watch: {
		subsDate() {
			...
			this.loading = true
			Promise.all([this.loadAnnualMetrics(), this.loadMonthlyMetrics()]).then(() => this.loading = false)
		}
	},
	computed: {
		labels() {
		...
		series() {
			return this.metricNames.map(metric => ({
				...
				data: this.labels.map(label => {
					const match = this.metrics.monthly.find(d =>
						d.metric === metric &&
						moment(`${d.subscriptionYear}-${d.subscriptionMonth}`, 'YYYY-M').format('MMMM YYYY') === label
					)
		...
		monthlyItems() {
			...
			this.metrics.monthly.forEach(item => {
				const monthKey = moment(`${item.subscriptionYear}-${item.subscriptionMonth}`, 'YYYY-M')
					.format('MMMM YYYY')
				if (!grouped[monthKey]) {
		...
		yearlyItems() {
			...
			if (sortedGroup.length > 0) {
				...
				sortedGroup[0].year = this.$t('admin.system.execution-metrics.fromUpToToday', { from: subsDate })
				sortedGroup[1].year = this.$t('admin.system.execution-metrics.fromTo', { from: prevDate, to: subsDate })
			}
			return sortedGroup
		}
	},
	...
	methods: {
		...
		loadMonthlyMetrics() {
			const params = {
				...
				startDate: moment(this.subsDate).subtract(1, 'years').startOf('day').format('YYYY-MM-DD[T]HH:mm:ss.SSSZZ')
			}
			return SystemService.getMetricsData(params).then(data => {
