  ...
  <div class="d-flex flex-column bg-light" :style="{ height: 'calc(100% - 55px)' }">
      ...
      <div class="row align-items-center pb-2">
        <div class="col-4">
          <b-input-group size="sm">
            ...
            <b-form-input :title="$t('searches.search')" :placeholder="$t('searches.search')" @keyup="searchGroups" v-model="filter"></b-form-input>
          </b-input-group>
        </div>
    ...
    <div class="container overflow-auto bg-white shadow g-0" @scroll="showMore">
      <FlowTable thead-class="sticky-header" striped :items="groups" primary-key="id"
        ...
        <template v-slot:cell(actions)="row">
          <div v-if="$root.config.userProvider === 'org.cibseven.auth.SevenUserProvider'">
    ...
    <b-modal ref="deleteModal" :title="$t('confirm.title')">
      ...
      <template v-slot:modal-footer>
        <b-button variant="primary" @click="remove(groupSelected); $refs.deleteModal.hide()">{{ $t('confirm.ok') }}</b-button>
