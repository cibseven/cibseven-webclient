...
public class BatchService extends BaseService implements InitializingBean {
	...
	public Collection<Batch> getBatches(
		...
		checkPermission(user, BATCH, PermissionConstants.READ_ALL);
    ...
    @DeleteMapping("/{id}")
   	public void deleteBatch(
			...
			HttpServletRequest rq) {
		CIBUser user = checkAuthorization(rq, true, false);
		checkPermission(user, BATCH, PermissionConstants.DELETE_ALL);
    ...
    @PutMapping("/{id}/suspended")
   	public void setBatchSuspensionState(
			...
			HttpServletRequest rq) {
		CIBUser user = checkAuthorization(rq, true, false);
		checkPermission(user, BATCH, PermissionConstants.UPDATE_ALL);
