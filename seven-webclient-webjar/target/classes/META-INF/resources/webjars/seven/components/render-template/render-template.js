/* globals window, localStorage, permissionsMixin */
	...
	const RenderTemplate = {
		template: '<div>\
			<loader v-if="loader" class="h-100 d-flex justify-content-center" ref="loader" styling="width:20%"></loader>\
			<div v-show="!loader" class="h-100">\
				...
				<div class="pt-2" v-if="!formFrame">\
					...
					<icon-button icon="check" @click="completeEmptyTask()" variant="secondary" :text="$t(\'task.actions.submit\')"></icon-button>\
				</div>\
				<success top="0" style="z-index: 1031" ref="messageSaved"> {{ $t(\'alert.successSaveTask\') }}</success>\
				<success top="0" style="z-index: 1031" ref="messageSuccess"> {{ $t(\'alert.successOperation\') }}</success>\
			</div></div>',
		props: ['task'],
