...
/**
 * The Spring Boot 4 flavour of the modeler persistence unit has to hold the same guarantees as the
 * Boot 3 one (CIB7-1776): distinct bean names, nothing {@code @Primary}, and no modeler entities in
 * the host's unit. Boot 4 moved {@code EntityManagerFactoryBuilder} to {@code org.springframework.boot.jpa}
 * and split JPA auto-configuration into its own modules, so the wiring is verified per flavour
 * rather than assumed from the Boot 3 test in {@code cibseven-webclient-web}.
 */
